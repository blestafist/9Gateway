...
func TestSystemDirectConstructorWiring(t *testing.T) {
	database, err := storage.Open(context.Background(), ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !payload.Ready {
		t.Errorf("expected Ready = true from direct NewHandlerWithAdmin constructor")
	}
	if !payload.Storage.Healthy {
		t.Errorf("expected Storage.Healthy = true")
	}
	for _, check := range []string{"sqlite", "schema", "upstream", "lifecycle"} {
		if c, ok := payload.Readiness.Checks[check]; !ok || c.Status != "pass" {
			t.Errorf("expected readiness check %q to pass, got: %v", check, c)
		}
	}
}
