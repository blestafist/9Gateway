-- Version 9 tightens request lifecycle ordering and adds the completion-time
-- access path used by both independent retention passes. Version 7 and 8 are
-- recent, but this is still a real migration so existing databases receive the
-- same constraints as fresh databases.
--
-- SQLite cannot add table CHECK constraints with ALTER TABLE. Rebuild both
-- related tables so request_bodies remains foreign-key safe while requests is
-- replaced, preserving all rows and migration transactionality.
ALTER TABLE requests RENAME TO requests_v8;
ALTER TABLE request_bodies RENAME TO request_bodies_v8;

DROP INDEX idx_requests_recent;
DROP INDEX idx_requests_key_time;
...
CREATE TABLE requests (
    ...
    CHECK (started_at IS NULL OR upstream_started_at IS NULL OR upstream_started_at >= started_at),
    CHECK (upstream_started_at IS NULL OR upstream_headers_at IS NULL OR upstream_headers_at >= upstream_started_at),
    CHECK (upstream_headers_at IS NULL OR first_byte_at IS NULL OR first_byte_at >= upstream_headers_at),
    CHECK (first_byte_at IS NULL OR finished_at IS NULL OR finished_at >= first_byte_at),
    CHECK (
...
INSERT INTO requests SELECT * FROM requests_v8;

CREATE INDEX idx_requests_recent
    ON requests(started_at DESC, request_id);
CREATE INDEX idx_requests_finished
    ON requests(finished_at ASC, request_id);
...
INSERT INTO request_bodies SELECT * FROM request_bodies_v8;

DROP TABLE request_bodies_v8;
DROP TABLE requests_v8;
