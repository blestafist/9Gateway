...
This file contains the active queue and the just-completed T036-T040 batch until
the next planning pass. Read `AGENTS.md`, `CURRENT.md`, and the single task being
implemented. Do not load the whole cold backlog. Completed tasks below are kept
as the current acceptance record and must not be selected again. When fewer than
five active tasks remain, promote the next coherent batch from
...
### T036 - Parse SSE data lines and event names (complete)
...
### T037 - Handle SSE comments and field syntax safely (complete)
...
### T038 - Parse split and coalesced input reads (complete)
...
### T039 - Bound SSE event memory (complete)
...
### T040 - Add a reusable streaming test upstream (complete)
