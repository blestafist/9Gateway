# Full backlog — Atomic implementation plan

> Cold long-range backlog preserved from initial planning. `TASKS.md` at the
> repository root is the active queue and is the only task list read during
> routine implementation. Promote tasks from here when the active queue gets
> short, rewriting them into the compact active-task format and adding only
...
Поля согласно `docs/architecture/storage.md#api-keys`.
...
The original rules were promoted and condensed into root `AGENTS.md`. That file
is the sole active source for agent workflow and required context. Do not infer
routine reading requirements from this cold backlog.
