...
export const KeyDetailDrawer: React.FC<KeyDetailDrawerProps> = ({
  ...
  return (
    <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
              {detail && mode === "view" ? (
                <Button
                  variant="primary"
                  size="sm"
                  leftIcon={<Edit3 size={14} aria-hidden="true" />}
                  onClick={() => setMode("edit")}
                  data-testid="open-edit-policy-btn"
                >
                  Edit Policy
                </Button>
              ) : <div />}
      ...
      >
        ...
        {!isLoading && !isError && detail && statusInfo && (
          <div className="gw-key-detail-content" data-testid="key-detail-content">
            ...
            {/* Section 3: Policy Form in Edit mode, or Policy Summary in View mode */}
            {mode === "edit" ? (
              <section className="gw-key-detail-section" aria-labelledby="detail-policy-title">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-2)" }}>
                  ...
                  <Button
                    ...
                    data-testid="switch-to-view-mode-btn"
                  >
                    View Summary
                  </Button>
                </div>
      ...
      {/* Discard Confirmation Dialog for Drawer Close */}
      <Dialog
        isOpen={isDiscardDialogOpen}
        onClose={() => setIsDiscardDialogOpen(false)}
        title="Discard Unsaved Changes?"
        description="You have unsaved policy modifications. Leaving will discard your edits."
        footer={
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "var(--space-2)", width: "100%" }}>
            <Button
              variant="secondary"
              onClick={() => setIsDiscardDialogOpen(false)}
              data-testid="drawer-discard-cancel-btn"
            >
              Keep Editing
            </Button>
            <Button
              ...
              data-testid="drawer-discard-confirm-btn"
            >
              Discard Changes
            </Button>
          </div>
      ...
      >
        <p style={{ fontSize: "var(--font-size-sm)", color: "var(--text-secondary)", margin: 0 }}>
          Closing the key detail drawer will revert any unsaved changes to model rules, rate limits, budgets, concurrency, or logging settings.
        </p>
      </Dialog>
    </>
  );
